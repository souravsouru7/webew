...
export default function PremiumLandingPage() {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-black">
      ...
      <div className="relative z-10 h-full flex items-center justify-center px-8">
        <div className="max-w-6xl w-full text-center">
          {sections.map((section, index) => (
            <div
              key={section.id}
              className="absolute inset-0 flex items-center justify-center px-8 transition-opacity duration-1000 ease-out"
              style={{
                opacity: currentIndex === index ? 1 : 0,
            ...
            >
              <div className="max-w-5xl">
                <div className="mb-4 text-white/60 text-sm font-medium tracking-widest uppercase">
                  {section.id}
                </div>
                
                <h1 className="text-7xl md:text-8xl lg:text-9xl font-bold text-white mb-4 tracking-tight leading-none">
                  {section.title}
                </h1>
                
                <p className="text-3xl md:text-4xl lg:text-5xl text-white/90 font-light mb-6">
                  {section.subtitle}
                </p>
                
                <p className="text-lg md:text-xl lg:text-2xl text-white/70 mb-12 max-w-3xl mx-auto">
                  {section.description}
                </p>
                
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center">
                  <button className="px-10 py-4 bg-white text-black text-base font-semibold rounded-full hover:bg-gray-200 transition-all transform hover:scale-105 shadow-xl">
                    {section.cta1}
                  </button>
                  <button className="px-10 py-4 border-2 border-white text-white text-base font-semibold rounded-full hover:bg-white/20 transition-all transform hover:scale-105">
                    {section.cta2}
                  </button>
                </div>
              </div>
      ...
      {/* Progress Navigation */}
      <div className="fixed right-6 md:right-10 top-1/2 -translate-y-1/2 z-20 flex flex-col gap-3">
        {sections.map((section, index) => (
          <button
            key={section.id}
            onClick={() => {
          ...
          >
            ...
            />
            <span className="absolute right-6 top-1/2 -translate-y-1/2 bg-white text-black px-3 py-1 rounded text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
              {section.id.charAt(0).toUpperCase() + section.id.slice(1)}
            </span>
          </button>
        ))}
      ...
      {currentIndex < sections.length - 1 && (
        <div className="fixed bottom-8 md:bottom-10 left-1/2 -translate-x-1/2 z-20 animate-bounce">
          <div className="text-white/50 text-xs md:text-sm mb-2 text-center font-medium">
            Scroll to explore
          </div>
          <svg className="w-5 h-5 md:w-6 md:h-6 text-white/50 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Section Counter */}
      <div className="fixed bottom-8 md:bottom-10 left-8 md:left-10 z-20 text-white/60 text-sm font-medium">
